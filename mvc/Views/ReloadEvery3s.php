 		...
 		<div class="text-center" style="width: 300px; ">
 			<div class="row" id="alert-message" style="width: 100%;">
 				<?php for($i = 0; $i < count($users); $i++){ ?>
 					<?php if($messageModel->countMessageNoRead($users[$i]) > 0){ ?>
 						<p class="ml-4 text-danger"><?php echo $messageModel->countMessageNoRead($users[$i]); ?> tin nhắn mới từ <?php echo $userModel->showUser($users[$i])->name; ?></p>
 					<?php }} ?>
 			</div>
 		</div>
 	</div>
